...
func renderLogin(c *gin.Context) {
	...
	if is, ok := isAuthenticated.(bool); ok && is {
		log.Printf("user authenticated in handlerLogin.go:18")
		...
		log.Printf("user NOT authenticated in handlerLogin.go:16")
		injectSocials(c)
		log.Printf("after socials in handlerLogin.go:33")
		c.HTML(http.StatusOK, c.Request.URL.Path, c.Keys)
		log.Printf("after HTML in handlerLogin.go:35")
	}
}

func login(c *gin.Context) {
	log.Printf("error in handlerLogin.go:37")
