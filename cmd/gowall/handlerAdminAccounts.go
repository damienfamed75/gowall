...
func renderAccounts(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:51")
		EXCEPTION(err)
	}
...
func createAccount(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:86")
		EXCEPTION(err)
	}
	if len(body.Name) == 0 {
...
func readAccount(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:160")
		EXCEPTION(err)
	}
	if XHR(c) {
...
func updateAccount(c *gin.Context) {
	...
	err := collection.FindId(bson.ObjectIdHex(c.Param("id"))).One(account)
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:195")
		EXCEPTION(err)
	}
...
func linkUserToAccount(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:227")
		EXCEPTION(err)
	}
...
func newNote(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:362")
		EXCEPTION(err)
	}
	if len(body.Data) == 0 {
...
func newStatus(c *gin.Context) {
	...
	err := decoder.Decode(&body)
	if err != nil {
		log.Println("error at handlerAdminAccounts.go:411")
		EXCEPTION(err)
	}
