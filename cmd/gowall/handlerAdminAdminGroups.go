...
func renderAdminGroups(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAdminGroups.go:43")
		EXCEPTION(err.Error())
	}
...
func createAdminGroup(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAdminGroups.go:99")
		EXCEPTION(err)
	}
	response.Finish()
}
...
func readAdminGroup(c *gin.Context) {
	...
	if err != nil {
		log.Println("error at handlerAdminAdminGroups.go:120")
		EXCEPTION(err)
	}
	if XHR(c) {
		c.Header("Cache-Control", "no-cache, no-store, must-revalidate")
