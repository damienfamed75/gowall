...
func signup(c *gin.Context) {
	...
	if err != nil {
		log.Printf("error in handlerSignup.go:41")
		EXCEPTION(err)
	}
	...
	if response.HasErrors() {
		log.Printf("error in handlerSignup.go:51")
		response.Fail()
		return
	}
	...
	if err != nil && err != mgo.ErrNotFound {
		log.Printf("error in handlerSignup.go:68")
		EXCEPTION(err)
	}
	...
	if response.HasErrors() {
		log.Printf("error in handlerSignup.go:83")
		response.Fail()
		return
	}
	...
	if err != nil {
		log.Printf("error in handlerSignup.go:105")
		EXCEPTION(err)
	}
